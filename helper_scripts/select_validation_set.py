...
file_list = listdir(join(baseDir, fourier_re_folder))
...
valid_fourier_re_dir_path = join(valid_dir_path, fourier_re_folder)
valid_fourier_im_dir_path = join(valid_dir_path, fourier_im_folder)
valid_in_dir_path = join(valid_dir_path, in_folder)
valid_out_dir_path = join(valid_dir_path, out_folder)


for dir in [valid_fourier_re_dir_path, valid_fourier_im_dir_path, valid_in_dir_path, valid_out_dir_path]:
    if not exists(dir):
        makedirs(dir)
...
for i in range(N_VALID):
    ## get files 
    fourier_re_file = join(join(baseDir, fourier_re_folder), file_names[i])
    fourier_im_file = join(join(baseDir, fourier_im_folder), file_names[i])
    in_file = join(join(baseDir, in_folder), file_names[i])
    out_file = join(join(baseDir, out_folder), file_names[i])
    
    new_fourier_re_file = join(valid_fourier_re_dir_path, file_names[i])
    new_fourier_im_file = join(valid_fourier_im_dir_path, file_names[i])
    new_in_file = join(valid_in_dir_path, file_names[i])
    new_out_file =join(valid_out_dir_path, file_names[i])
    ## move the files
    rename(fourier_re_file, new_fourier_re_file)
    rename(fourier_im_file, new_fourier_im_file)
